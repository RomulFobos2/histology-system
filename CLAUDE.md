...
- Перед новой задачей синхронизируй master: `git checkout master && git pull --ff-only`. Только после этого `git checkout -b claude/{описание}` — **новая ветка всегда создаётся от свежего origin/master**.
- **Никогда** не базируй новую ветку или PR на feature-ветке другого незакрытого PR. Если новая задача логически зависит от ещё не смерженного PR — дождись его merge в master либо явно согласуй порядок с пользователем.
- При создании PR через `gh pr create` всегда явно указывай `--base master`. Без этого `gh` может выбрать непредсказуемую базу (например, текущую upstream-ветку), и изменения попадут не туда.
...
- После merge PR удаляй устаревшие ветки на remote (`git push origin --delete <branch>`) и локально (`git branch -D <branch>`), чтобы GitHub не показывал баннер «had recent pushes»
