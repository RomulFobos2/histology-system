...
### Обоснование (бизнес-контекст)
В ГБУ «БСМЭ» для гистологических исследований используется оптический микроскоп **Nikon Eclipse E200** — стандартный лабораторный микроскоп для рутинной гистологии. Изображения с него получаются через окулярную камеру или фотоадаптер, и их качество не всегда идеально: возможны шум камеры, расфокусировка, неравномерное освещение, артефакты сжатия при сохранении. Для точной диагностики в судебно-медицинской экспертизе критически важно видеть мельчайшие клеточные структуры. Автоэнкодер решает задачу автоматического улучшения таких изображений: убирает шум, повышает резкость, восстанавливает детали, что помогает врачу-гистологу ставить более точные диагнозы.

### Датасеты для обучения

Модель использует self-supervised обучение: берёт чистые изображения, синтетически деградирует их (шум, размытие, JPEG-артефакты, потеря контраста) и учится восстанавливать оригинал. Пары «хороший-плохой» НЕ нужны — достаточно любых качественных гистологических снимков.

**Собственные данные (94 снимка):**
- `autoencoder/data/Препарат1/` — 23 JPG (окулярная камера, Nikon Eclipse E200)
- `autoencoder/data/Препарат3/` — 39 TIF (полноразмерные)
- `autoencoder/data/Препарат4/` — 32 TIF (полноразмерные)

**Внешний датасет — NCT-CRC-HE-100K:**
- 100 000 изображений H&E-окрашенных тканей (колоректальный рак + норма), 224×224 px
- Источник: Zenodo — https://zenodo.org/records/1214456
- Лицензия: CC BY 4.0
- 9 классов тканей: ADI, BACK, DEB, LYM, MUC, NORM, STR, TUM
- Скачать и распаковать в `autoencoder/data/NCT-CRC-HE-100K/`
- Код автоматически найдёт все изображения (рекурсивный поиск в `data/`)

**Стратегия обучения:** обучить модель на комбинированном датасете (~100k изображений), затем продемонстрировать улучшение на реальных снимках из лаборатории БСМЭ.
